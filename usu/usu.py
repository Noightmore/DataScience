...
def draw(x, y, title, x_label, y_label, *args):
    ...
    if len(args) >= 1:
        ...
        if len(args) == 3:
            value_x = float(args[1])
            value_y = float(args[2])

            if np.min(x_line) <= value_x <= np.max(x_line):
                plt.axvline(x=value_x, color='red', linestyle=':')

            if np.min(y_pred) <= value_y <= np.max(y_pred):
                plt.axhline(y=value_y, color='red', linestyle=':')
                plt.plot(value_x, value_y, marker='.', color='red', markersize=8)
            print(f"x : {value_x:.3f}, y : {value_y:.3f}")
...
def draw2d(train_data, train_targets, test_data, test_targets, etalons=None, title='Data', x_label='x', y_label='y'):
    ...
    plt.scatter(train_data.T[0], train_data.T[1], s=40, marker='o', c=train_targets, cmap=color_point_train,
                label="trainData")
    plt.scatter(test_data.T[0], test_data.T[1], s=40, marker='o', c=test_targets, cmap=color_point_test,
                edgecolors='black', label="testData")
...
def checkSoftmax(softmax):
    ...
    return np.allclose(output_torch, np.array(output))
