function LTWtest(referenceFile, fileListPath, method)
    ...
    %  example usage: LTWtest('LTW_reference_features_all.mat', 'TestFileList.txt', 3);
    ...
    for i = 1:numTestFiles
        ...
        for j = 1:length(referenceEnergy)
            switch method
                case 1 % Just Energy
                    distances(j) = LTWDistance(energySeq, referenceEnergy{j}, windowSize);
                case 2 % Energy + ZCR
                    distEnergy = LTWDistance(energySeq, referenceEnergy{j}, windowSize);
                    distZCR = LTWDistance(zcrSeq, referenceZCR{j}, windowSize);
                    distances(j) = distEnergy + distZCR;
                case 3 % K Spectral Features
                    distances(j) = LTWDistance(spectralSeq, referenceSpectral{j}, windowSize);
                case 4 % All Features
                    distEnergy = LTWDistance(energySeq, referenceEnergy{j}, windowSize);
                    distZCR = LTWDistance(zcrSeq, referenceZCR{j}, windowSize);
                    distSpectral = LTWDistance(spectralSeq, referenceSpectral{j}, windowSize);
