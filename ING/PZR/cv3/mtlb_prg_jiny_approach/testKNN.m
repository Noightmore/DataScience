function testKNN(referenceFile, fileListPath, method)
    % KNNtest - Tests speech recognition using KNN and appends results to an Excel file.
    %
    % Usage:
    %   KNNtest('KNN_reference_features.mat', 'TestFileList.txt', 1);
    %
    % Inputs:
    ...
    metricNames = ["Just Energy", "Energy + ZCR", "K Spectral Analysis", "All Features"];
    ...
    for i = 1:numTestFiles
        ...
        % Compute ZCR, Energy & Spectral Features
        logEnergy = log(sum(frames.^2));
        zcr = sum(abs(diff(sign(frames)))) / (2 * frameSize);
        spectralFeaturess = spectralFeatures(frames, 16); % 16 spectral bands

        % Compute Euclidean distance based on selected method
        switch method
            case 1 % Only Energy
                distances = sqrt((referenceEnergy - mean(logEnergy)).^2);
            case 2 % Energy + ZCR
                distances = sqrt((referenceEnergy - mean(logEnergy)).^2 + (referenceZCR - mean(zcr)).^2);
            case 3 % K Spectral Analysis
                distances = sqrt(sum((referenceSpectral - spectralFeaturess).^2, 1));
            case 4 % All Features
                distZCR = (referenceZCR - mean(zcr)).^2;
                distEnergy = (referenceEnergy - mean(logEnergy)).^2;
                distSpectral = sum((referenceSpectral - spectralFeaturess).^2, 1);
                distances = sqrt(distZCR + distEnergy + distSpectral);
        end
